/**
 * Total-ECH Pages 完整版（已修复 cf 参数 bug，统一 CF 静态域名）
 * 路由：
 *   /         -> 查询页面
 *   /api/query -> JSON API
 *   /ech      -> DoH 注入 ECH
    ...
    try {
        ...
        if (isStaticCF || isStaticMeta) {
            ...
            if (qType === 1) {
                ...
                // CF 静态域名：优先使用 ip4 参数，其次 cfDomain，最后默认 CF IP
                if (isStaticCF) {
                    if (config.ip4) {
                ...
                // Meta 静态域名：优先使用 metaIp4 参数，否则默认 Meta IP
                if (isStaticMeta) {
                    if (config.metaIp4) {
        ...
        if (qType === 1 || qType === 28) {
            ...
            // CF 域名（CIDR 探测）才允许使用 cfDomain 替换 IP
            if (ownerData === 'CF') {
                // Meta 相关 ip 参数优先于 cfDomain
            ...
            if (ownerData === 'META') {
                ...
                // 对于 Meta，若无自定义 IP，则尝试使用探测到的 IP 或正常转发
                let rawIps = [];
...
<body>
    <div class="container">
        <h1>🔒 ECH DNS 查询</h1>
        <p class="subtitle">自动注入 ECH，支持自定义 Cloudflare / Meta 优选 IP</p>
        <div>
