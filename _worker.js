    ...
    if (type === 'HTTPS') {
        if (isStatic) {
            ...
            if (effectiveCF) {
                ...
                const httpsRecord = packHttpsParamsWithHints(1, ".", [{ key: 'alpn', val: alpn }, { key: 'ech', val: ech || '' }], ipv4Hints, ipv6Hints);
...
async function handleStaticDomain(domain, type, config, isCF, isMeta, clientIP) {
    // 仅处理 A/AAAA，HTTPS 已在 resolveDNS 中单独处理
    if (type === 'AAAA') {
...
async function collectIpHints(domain, config, clientIP, owner, enhanceMode) {
    ...
    if (enhanceMode === 'full') {
        // full 模式：自动获取 hints，但仅对非静态域名（owner 为 null）
        if (!owner) {
...
// 其余工具函数（parseIpList, shuffle, resolveMultiDomainToIps, queryUpstreamDNS, fetchRealEch, encodeSvcParam 等）保持不变
// 特别注意：encodeSvcParam 中 Base64 解码已经通过 try-catch 容错
    ...
    <div class="container">
        ...
        <div class="global-section">
            <div class="param-grid">
                ...
                <div>
                    <label>自定义 Client IP (ECS) <span style="font-weight:normal;font-size:0.8em;">留空自动获取</span></label>
                    <input type="text" id="clientIp" placeholder="8.8.8.8 或 IPv6" value="">
                </div>
            </div>
        </div>
